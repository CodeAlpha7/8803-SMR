    ...
    for i in range(sliceNo):
        ...
        for j in range(resNo):
            tmp_utility[j], x[i, j], tmp_real_utility[j] = \
                simple_static_alogrithm(
                    z_minus_u=Rmax / sliceNo,
                    alpha=alpha[i, j],
                    weight=weight[i],
                    UENum=ueNo,
...
def admm_ddpg_algorithm(sliceNo, ueNo, resNo, alpha, weight, idx, model_path):
    # ddpg allocation
...
def admm_td3_algorithm(sliceNo, ueNo, resNo, alpha, weight, idx, model_path):
    # td3 allocation
    ...
    for ite in range(ADMM_iter):
        ...
        for i in range(sliceno):
            ...
            for j in range(resno):
                ...
                tmp_utility[j], x[i, j], tmp_real_utility[j] = \
                    simple_convex_alogrithm(
                        z_minus_u=z_minus_u[i, j],
                        alpha=alpha[i, j],
                        weight=weight[i],
                        UENum=ueno,
                        minReward=minReward / maxTime
...
def admm_mix_algorithm(SliceNum, UENum, RESNum, alpha, weight, simulated_optimization, model_path):
    #  ddpg allocation
...
def main_admm_algorithm(SliceNum, UENum, RESNum, alpha, weight, INDEX, model_path):
    utility_static = admm_static_algorithm(sliceNo=SliceNum, ueNo=UENum, resNo=RESNum, alpha=alpha, weight=weight)

    utility_ddpg = admm_ddpg_algorithm(sliceNo=SliceNum, ueNo=UENum, resNo=RESNum, alpha=alpha, weight=weight, idx=INDEX, model_path=model_path)[-1]
