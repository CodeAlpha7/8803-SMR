...
def admm_ddpg_algorithm(SliceNum, UENum, RESNum, alpha, weight, INDEX):
    ...
    for ite in range(ADMM_iter):
        ...
        print('iter ' + str(ite))
        print('current allocation via ddpg is')
        print(sumx)
...
def admm_opt_algorithm(SliceNum, UENum, RESNum, alpha, weight):
    ...
    for ite in range(ADMM_iter):
        ...
        print('iter ' + str(ite))
        print('current allocation via opt is')
        print(sumx)
...
def admm_mix_algorithm(SliceNum, UENum, RESNum, alpha, weight, simulated_optimization):
    ...
    for ite in range(ADMM_iter):
        ...
        print('iter ' + str(ite))
        print('current allocation via mix is')
        print(sumx)
...
if __name__ == "__main__":
    ...
    Times_Vec = [20] * SliceNum # [20,20,20]

    for iSlice in range(len(Times_Vec)):

        print(iSlice)
        Times = Times_Vec[iSlice] # 20
        sliceno = iSlice # 0
        iseed = 0
        ...
        for ite in range(Times):
            ...
            INDEX = np.random.choice(V, SliceNum, replace=True)
